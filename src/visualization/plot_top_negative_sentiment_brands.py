...
# Filter top 10 by negative sentiment ratio
df_top_negative = df.sort_values(by="negative_ratio", ascending=False).head(10)

# Save data to JSON
output_data = df_top_negative[["negative", "neutral", "positive", "total", "negative_ratio"]].to_dict(orient="index")
output_path = Path("data/visualizations")
output_path.mkdir(parents=True, exist_ok=True)
json_file = output_path / "top_negative_sentiment_brands_data.json"
with open(json_file, "w", encoding="utf-8") as f:
    json.dump(output_data, f, indent=2)

# Plot horizontal bar chart
df_top_negative[["negative_ratio"]].plot(
    kind="barh",
    figsize=(10, 6),
    color="red",
    legend=False
...
plt.title("Top 10 Car Brands by Negative Sentiment Ratio")
plt.tight_layout()

# Save plot image
img_file = output_path / "top_negative_sentiment_brands.png"
plt.savefig(img_file)
plt.show()

print(f"✅ Negative sentiment ratio chart saved to {img_file}")
