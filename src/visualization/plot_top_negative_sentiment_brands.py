...
df_filtered["negative_ratio"] = df_filtered["negative"] / df_filtered["total"]
df_filtered = df_filtered.sort_values(by="negative_ratio", ascending=False).head(10)

# Save filtered data to JSON
output_data = df_filtered[["negative", "neutral", "positive", "total", "negative_ratio"]].to_dict(orient="index")
with open("data/visualizations/negative_sentiment_ratio.json", "w", encoding="utf-8") as f:
    json.dump(output_data, f, indent=2)

# Plot
df_filtered[["negative", "neutral", "positive"]].plot(
    kind="barh",
    stacked=True,
    figsize=(10, 6),
...
plt.xlabel("Mentions")
plt.title("Top 10 Car Brands by Negative Sentiment Ratio (≥10 mentions)")
plt.tight_layout()

output_path = Path("data/visualizations")
output_path.mkdir(parents=True, exist_ok=True)
plt.savefig(output_path / "negative_sentiment_ratio.png")
plt.show()

print("✅ Negative sentiment ratio chart and data saved.")